 ...
 */
const IMAGE_GLYPHS = [
  { glyph: "✓", pattern: /checkmark/i },
  { glyph: "✗", pattern: /crossout|xmark/i },
];
...
const DROPPED_PLATFORMS = new Set(["tvOS", "visionOS", "watchOS"]);
...
function listedPages(collection) {
  ...
  return identifiers
    ...
    .filter(Boolean)
    .map((ref) => ({
      slug: ref.url?.match(HIG_URL_RE)?.[1],
      role: ref.role,
      title: ref.title,
    }))
...
function isDroppedPlatformText(text) {
  ...
  return (
    mentioned.length > 0 &&
    mentioned.every((token) => DROPPED_PLATFORMS.has(token))
  );
}

function isDroppedPlatformLabel(text) {
  return (
    PLATFORM_LIST_RE.test((text ?? "").trim()) && isDroppedPlatformText(text)
  );
}
...
function relabelHeading(text) {
  ...
  const kept = platformsMentioned(trimmed).filter(
    (token) => !DROPPED_PLATFORMS.has(token)
  );
...
function renderReference(node, ctx) {
  ...
  const title =
    node.overridingTitle ?? ref?.title ?? node.identifier.split("/").pop();
  const url = ref?.url;
  if (!url) {
    return title;
  }
  ...
  if (pathPart === "" && anchor) {
    return renderHigLink(ctx.slug, anchor, title, ctx);
  }
  const slug =
    pathPart.match(HIG_URL_RE)?.[1] ?? url.match(HIG_ABSOLUTE_RE)?.[1];
  if (slug) {
    return renderHigLink(slug, anchor, title, ctx);
  }
  return url.startsWith("/")
    ? `[${title}](${DEVELOPER_BASE}${url})`
    : `[${title}](${url})`;
}
  ...
  const walk = blocks.reduce(
    (state, block) => {
      if (block.type === "heading") {
        ...
        const dropped =
          droppedContext || insideSkipped || isDroppedPlatformText(block.text);
        const entry = block.anchor
          ? [
              [
                block.anchor.toLowerCase(),
                dropped ? "" : githubSlug(relabelHeading(block.text)),
              ],
            ]
          : [];
        const skipUntil = insideSkipped
          ...
          : isDroppedPlatformText(block.text)
            ? block.level
            : null;
        return { entries: [...state.entries, ...entry], skipUntil };
      }
      if (block.type === "tabNavigator") {
        ...
        const nested = (block.tabs ?? []).flatMap((tab) =>
          anchorEntries(
            tab.content ?? [],
            skipped || isDroppedPlatformText(tab.title)
          )
        );
...
function anchorMapFor(json) {
  const content =
    json.primaryContentSections?.find((section) => section.kind === "content")
      ?.content ?? [];
...
function resolveAnchor(slug, anchor, ctx) {
  ...
  if (mapped === undefined) {
    ctx.staleAnchors.add(`${ctx.slug}.md -> ${slug}.md#${lowered}`);
  }
  return mapped ?? "";
}
...
function renderLink(node, ctx) {
  ...
  const label =
    renderInline(node.titleInlineContent, ctx) || node.title || destination;
  const hig = destination.match(HIG_ABSOLUTE_RE);
  if (hig) {
    return renderHigLink(hig[1], hig[2], label, ctx);
  }
...
function renderImage(node, ctx) {
  ...
  const glyph = IMAGE_GLYPHS.find((entry) =>
    entry.pattern.test(node.identifier ?? "")
  );
  if (glyph) {
    return glyph.glyph;
  }
  ...
  switch (node.type) {
    ...
    default: {
      return node.inlineContent
        ? renderInline(node.inlineContent, ctx)
        : (node.text ?? "");
    }
  }
}
...
function renderParagraph(block, ctx) {
  ...
  if (!text || isDroppedBoilerplate(text)) {
    return [];
  }
  return [text, ""];
}
...
function renderAside(block, ctx, level) {
  const label =
    block.name ??
    (block.style
      ? block.style.charAt(0).toUpperCase() + block.style.slice(1)
      : "Note");
...
function resolveSpans(rows, extendedData = {}) {
  const spanOf = (r, c) =>
    extendedData[`${r}_${c}`] ?? { colspan: 1, rowspan: 1 };
  const originText = (r, c) => {
    ...
    if (span.rowspan === 0) {
      return originText(r - 1, c);
    }
    if (span.colspan === 0) {
      return originText(r, c - 1);
    }
    return rows[r]?.[c] ?? "";
  };
...
function renderTable(block, ctx) {
  ...
  const rows =
    block.header === "row" ? cells : [cells[0].map(() => ""), ...cells];
...
function renderTermList(block, ctx, level) {
  return (block.items ?? []).flatMap((item) => {
    ...
    const definition = trimBlankEdges(
      renderBlocks(item.definition?.content ?? [], ctx, level)
    );
    return [
      `**${term}**`,
      ...definition.map((line) => (line ? `  ${line}` : "")),
      "",
    ];
  });
}
...
function renderBody(blocks, ctx) {
  ...
  const final = blocks.reduce((state, block) => {
    if (block.type === "heading") {
      return reduceHeading(state, block);
    }
    if (state.skipUntil !== null) {
      return state;
    }
    if (state.inResources) {
      return state.resourcesSub === "Related"
        ? {
            ...state,
            related: [...state.related, ...collectReferences(block, ctx)],
          }
        : state;
    }
    return {
      ...state,
      lines: [...state.lines, ...renderBlock(block, ctx, state.level)],
    };
  }, initial);
...
function reduceHeading(state, block) {
  const unskipped =
    state.skipUntil !== null && block.level <= state.skipUntil
      ...
      : state;
  if (unskipped.skipUntil !== null) {
    return unskipped;
  }
  if (block.level === 2 && block.text === "Resources") {
    return { ...unskipped, inResources: true, resourcesSub: null };
  }
  if (unskipped.inResources && block.level > 2) {
    return { ...unskipped, resourcesSub: block.text };
  }
  const outside = unskipped.inResources
    ? { ...unskipped, inResources: false, resourcesSub: null }
    : unskipped;
  if (isDroppedPlatformText(block.text)) {
    return { ...outside, skipUntil: block.level };
  }
  return {
    ...outside,
    level: block.level,
...
function trimBlankEdges(lines) {
  ...
  const end =
    lines.length - [...lines].reverse().findIndex((line) => line !== "");
...
function uniqueBySlug(entries) {
  ...
  return entries.filter((entry) =>
    seen.has(entry.slug) ? false : seen.add(entry.slug)
  );
}
...
function renderPage(page, included, anchors, staleAnchors) {
  ...
  const relatedLines = related.length
    ? [
        "## Related guidelines",
        "",
        ...uniqueBySlug(related).map(
          (item) => `- [${item.title}](${item.slug}.md)`
        ),
        "",
      ]
    : [];
...
function renderLookup(included, omitted, pulledOn) {
  ...
  const curated = [
    ...
    ...CURATED_FILES.map(
      (item) =>
        `| [${item.title}](hig/${item.file}) | \`${item.file}\` | ${escape(item.covers)} |`
    ),
    "",
  ];
  const omittedTable = [
    ...
    ...omitted.map(
      (page) =>
        `| [${escape(page.title)}](${WEB_BASE}/${page.slug}) | ${escape(page.reason)} |`
    ),
    "",
  ];
  const intro = [
  ...
  return collapseBlankLines(
    [...intro, ...sectionTables, ...curated, ...omittedTable].join("\n")
  );
}
...
async function generatedFiles(outDir) {
  ...
  const flags = await Promise.all(
    entries
      ...
      .map(async (name) => {
        const head = new Set(
          (await readFile(path.join(outDir, name), "utf8")).slice(0, 400)
        );
        return head.has(`> Source: <${WEB_BASE}/`) && head.has("\n> Section: ")
          ? name
          : null;
      })
  );
...
async function pruneStaleFiles(outDir, keep, forcePrune) {
  const protectedFiles = new Set([
    ...keep,
    ...CURATED_FILES.map((item) => item.file),
  ]);
  const existing = await generatedFiles(outDir);
  const removable = existing.filter((name) => !protectedFiles.has(name));
  const limit = Math.max(
    PRUNE_COUNT_FLOOR,
    Math.floor(existing.length * PRUNE_SHARE_LIMIT)
  );
  if (removable.length > limit && !forcePrune) {
    return { removed: [], refused: removable };
  }
  await Promise.all(removable.map((name) => unlink(path.join(outDir, name))));
  return { refused: [], removed: removable };
}
...
async function main() {
  ...
  console.log(
    `Crawled ${entries.length} pages across ${SECTIONS.length} sections`
  );
  ...
  const pages = await mapWithConcurrency(
    ...
    async (entry) => {
      ...
      return {
        ...
        json,
        platforms,
        reason: omissionReason(entry.slug, platforms),
      };
    }
  );
  ...
  const anchors = new Map(
    included.map((page) => [page.slug, anchorMapFor(page.json)])
  );
  ...
  if (pruned.removed.length) {
    console.log(`Pruned stale files: ${pruned.removed.join(", ")}`);
  }
